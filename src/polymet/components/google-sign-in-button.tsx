...
export function GoogleSignInButton({ onSuccess }: GoogleSignInButtonProps) {
  ...
  if (user) {
    return (
      <Link
        to="/"
        className="inline-flex items-center justify-center w-full h-10 px-4 py-2 text-sm font-medium rounded-md bg-white hover:bg-gray-50 text-gray-900 border border-gray-300"
      >
        Go to Dashboard
      </Link>
    );
